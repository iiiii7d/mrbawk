# Mr Bawk - v1.8.1 (20/4/20) by i____7d
VERSION = "v1.8.1 (20/4/20)"
...
async def on_message(message):
    try:
        ...
        if msgLCase.startswith('parrot') and len(msgLCase.split(" ")) > 1 and msgLCase.split(" ")[1] in ["info", "help", "changelog", "ping", "list", "repeat", "stoprepeat", "detectswear", "stopdetectswear", "mute", "unmute", "inviteme", "myserver", "s", "honk", "q", "vcbawk", "c", "messup", "raise"] and message.author.bot == False:
            ...
            for h in members:
                if h.nick == None:
                    memberNicks.append(h.name)
                else:
                    memberNicks.append(h.nick)
                ...
                if len(command) > 2 and command[2] == "add":
                    if len(command) > 3 and int(msgLCase.replace("parrot q add ", "")) > 0:
                        try:
                            ...
                            if quote.content in [a["quote"] for a in quotesView(fullnameify(quote.author))[1]]:
                                ...
                                await client.get_channel(700946990678409276).send("**Message quoted by " + fullnameify(message.author).replace("_", "\_") + " (ID:" + str(message.author.id) + ") for " + fullnameify(quote.author).replace("_", "\_") + " (ID:" + str(quote.author.id) + "):** " + quote.content + " (ID: " + str(quoteCount()) + ")")
...
**- ...reset** Resets the list to its default.""")
        ...
        if not "discord.errors.Forbidden: 403 Forbidden (error code: 50001): Missing Access" in traceback.format_exc():
