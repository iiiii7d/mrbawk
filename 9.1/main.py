# Mr Bawk - v1.9.1 (24/4/20) by i____7d
VERSION = "v1.9.1 (24/4/20)"
...
Thread(target=memberlistsDriver).start()
print("MemberlistsDriver started")
...
def preferencesWrite(key, subkey, value):
    ...
    while preferencesQueue[code] == request:
        pass
    data = preferencesQueue[code]
    data[str(key)][str(subkey)] = value
    preferencesQueue.append({"request": "w", "new": data})

def preferencesDriver():
    cursor = 0
    while not client.is_closed():
...
Thread(target=preferencesDriver).start()
print("PreferencesDriver started")
...
def quotesView(memberfullname):
    ...
    if memberfullname == "anyone":
        member = random.choice(list(data))
        return [dividefullname(member)[0], data[member]]
    elif memberfullname in data:
...
Thread(target=quotesDriver).start()
print("QuotesDriver started")
...
Thread(target=offensiveDriver).start()
print("OffensiveDriver started")
...
Thread(target=currencyDriver).start()
print("CurrencyDriver started")
