...
#1.10.2 - 4/7/20 - checks() function removed
...
def currencyRead(bot, key, subkey):
    if bot == False:
        if len(list(db["currency"].find({"_id" : str(key)}))) == 0:
            db["currency"].insert_one({"_id": str(key), "balance": 50.0, "bowls": [{"id": 0, "inside": None, "attractsleft": 0}], "parrots": [],"inventory": [], "lastdaily": 0, "xp": 0, "level": 0, "notifs": [], "ach": [], "reactiongames": 0})
        else:
...
async def upXP(message, xpAdd):
    ...
    if currencyRead(message.author.bot, message.author.id, "xp") >= 100:
        levelsUp = 0
        while currencyRead(message.author.bot, message.author.id, "xp") >= 100:
            currencyWrite(message.author.bot, message.author.id, "xp", currencyRead(message.author.bot, message.author.id, "xp") - 100)
...
async def on_message(message):
    try:
        ...
        #print("i")
        #client.loop.create_task(checks(message))
        #print("j")
        
        msgLCase = message.content.lower()

        if msgLCase.startswith('parrot') and len(msgLCase.split(" ")) > 1 and msgLCase.split(" ")[1] in ["info", "help", "changelog", "cl", "ping", "list", "repeat", "re", "stoprepeat", "sre", "detectswear", "ds", "stopdetectswear", "sds", "mute", "unmute", "inviteme", "myserver", "s", "honk", "q", "c", "messup", "achievements", "ach", "uptime", "eval", "announce", "r", "gif", "vote"] and message.author.bot == False:
            print("a")
            ...
            if currencyRead(message.author.bot, message.author.id, "notifs") != []:
                string = ":parrot: **Since your last used my commands, you attracted: " + "".join([(str(p["qty"]) + " " + p["breed"] + "s, ") for p in currencyRead(message.author.bot, message.author.id, "notifs")])
                string = string[:-2] + "**"
...
- Parrot c pf now has an -a flag, to fill all or as much bowls as possible in one go.
- A new reaction test game! For more info do `parrot r`.
- New command: gif, for parrot gifs
- Nerfed some prices and tpms
-(.1) Optimised performance, the parrot shd be faster now
-(.1) `parrot s v` and `parrot list` should output lists in one message rather than 3 or 6
-(.2) More optimisation
-(.3) Fixed detectswear bug. Removed ping reaction
...
**stop** Stop an ongoing game. This command is for Parrot Masters only.""")
                ...
                if len(command) > 2 and command[2] == "daily":
                    if (currencyRead(message.author.bot, message.author.id, "lastdaily") + 86400000) < int(round(time.time() * 1000)):
                        currencyWrite(message.author.bot, message.author.id, "balance", currencyRead(message.author.bot, message.author.id, "balance") + 50)
                    ...
                    if toBuy in [p["name"].lower() for p in currstatsRead("shop")]:
                        ...
                        if toBuyStat["cost"]*qty > currencyRead(message.author.bot, message.author.id, "balance"):
                            await message.channel.send(":parrot: **Sorry, you don't have enough twigs for the " + toBuy + "!**")
                        else:
                            currencyWrite(message.author.bot, message.author.id, "balance", currencyRead(message.author.bot, message.author.id, "balance") - toBuyStat["cost"]*qty)
...
async def on_guild_join(guild):
    ...
    try:
        ...
        pass
    if len(list(db["memberlists"].find({"_id" : str(guild.id)}))) == 0:
        db["memberlists"].insert_one({"_id": str(guild.id), "toRepeat": [], "toDetectSwear": [], "toMute": []})

    if len(list(db["preferences"].find({"_id" : str(guild.id)}))) == 0:
