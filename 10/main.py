# Mr Bawk - v1.10 (11/6/20) by i____7d
VERSION = "v1.10 (11/6/20)"
...
#1.10 -  - Parrot c pf now has an -a flag, to fill all or as much bowls as possible in one go. Added reaction test game. Added giphy searching and gif command. Nerfed currstats. Added dbl stuff
...
- Append `-o` to mark it as offensive, which means that the quote can never be quoted again.""")
                ...
                if len(command) > 2 and command[2] == "start":
                    ...
                    for x in reactionGame[str(message.channel.id)]["players"]:
                        ...
                        currencyWrite(x["playerid"], "balance", currencyRead(x["playerid"], "balance") + twigs)
                        currencyWrite(x["playerid"], "reactiongames", currencyRead(x["playerid"], "reactiongames") + 1)
                        if currencyRead(x["playerid"], "reactiongames") >= 10:
                            ach(message.guild.get_member(x["playerid"]), 6)
                        if y == 1 and len(reactionGame[str(message.channel.id)]["players"]) >= 3:
                            ach(message.guild.get_member(x["playerid"]), 5)
...
**stop** Stop an ongoing game. This command is for Parrot Masters only.""")
                ...
                if len(command) > 2 and command[2] == "daily":
                    ...
                    for q in currencyRead(message.author.id, "bowls"):
                        if q["inside"] == None:
                            bowlIndex.append(currencyRead(message.author.id, "bowls").index(q))
...
**- ...reset** Resets the list to its default.""")
        ...
        if not "discord.errors.Forbidden: 403 Forbidden (error code: 50001): Missing Access" in traceback.format_exc() and not "discord.errors.Forbidden: 403 Forbidden (error code: 50013): Missing Permissions" in traceback.format_exc() and (message.content.startswith("parrot") or str(message.channel.id) in reactionGame.keys()):
            try:
...
dblclient = dbl.DBLClient(client, TOPGG)

@client.event
async def on_dbl_vote(data):
    currencyWrite(data["id"], "balance", currencyRead(data["id"], "balance") + 50)
