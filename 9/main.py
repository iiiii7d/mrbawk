# Mr Bawk - v1.9 (22/4/20) by i____7d
VERSION = "v1.9 (22/4/20)"
...
async def on_message(message):
    try:
        ...
        if msgLCase.startswith('parrot') and len(msgLCase.split(" ")) > 1 and msgLCase.split(" ")[1] in ["info", "help", "changelog", "cl", "ping", "list", "repeat", "re", "stoprepeat", "sre", "detectswear", "ds", "stopdetectswear", "sds", "mute", "unmute", "inviteme", "myserver", "s", "honk", "q", "c", "messup", "achievements", "ach"] and message.author.bot == False:
                    ...
                    with open(os.path.join(sys.path[0], "currency.json"), "r") as f:
                        ...
                        for m in servermemberids:
                            try:
                                ...
                                toSort[len(toSort)-1]["score"] = (toSort[len(toSort)-1]["balance"] + len(toSort[len(toSort)-1]["parrots"]) + toSort[len(toSort)-1]["level"]) / 10
                            except KeyError:
                                pass
                        ...
                        for m in toSort:
                            ...
                            if len(str(len(m["parrots"]))) > longestParrotLength:
                                longestParrotLength = len(str(len(m["parrots"])))
                        ...
                        f = 0
                        for t in toSort:
                            string = string + spacescontent(t["fullname"], nameHeader, " ") + "|" + spacescontent(t["balance"], balHeader, " ") + "|" + spacescontent(len(t["parrots"]), parrotlengthHeader, " ") + "|" + spacescontent(t["xp"], xpHeader, " ") + "|" + spacescontent(t["score"], scoreHeader, " ") + "\n"
                            f += 1
                            if f == 15:
                                break
                        string = string + "```"
...
Some actions give you xp. They are usually the action commands instead of the viewing commands. Each 100xp is converted into levels. Each level gives you a multiplier of +0.05%.
...
**- ...reset** Resets the list to its default.""")
        ...
        if not "discord.errors.Forbidden: 403 Forbidden (error code: 50001): Missing Access" in traceback.format_exc() and not "discord.errors.Forbidden: 403 Forbidden (error code: 50013): Missing Permissions" in traceback.format_exc():
            if "json.decoder.JSONDecodeError:" in traceback.format_exc():
                await message.channel.send(":parrot: Oops, please try again")
            else:
                try:
                    ...
                    if t["inside"] != None:
                        ...
                        if random.random() <= foodStat["cpm"]*multiplier:
                            with open(os.path.join(sys.path[0], "currency.json"), "r+") as f:
                                ...
                                d[u]["parrots"].append({"name": "Parrot #" + str(random.randint(0, 9999)), "breed": breed})
                                upXPSilent(u, round(1 - currstatsRead("parrots")[[a["name"] for a in currstatsRead("parrots")].index(breed)]["chance"] * 10))

                                if breed in [w["breed"] for w in d[u]["notifs"]]:
                                    d[u]["notifs"][[w["breed"] for w in d[u]["notifs"]].index(breed)]["qty"] += 1
                                else:
                                    d[u]["notifs"].append({"breed": breed, "qty": 1})

                                if d[u]["bowls"][users[u]["bowls"].index(t)]["attractsleft"] == 1:
                                    d[u]["bowls"][users[u]["bowls"].index(t)]["attractsleft"] = 0
                                    d[u]["bowls"][users[u]["bowls"].index(t)]["inside"] = None
                                else:
                                    d[u]["bowls"][users[u]["bowls"].index(t)]["attractsleft"] -= 1
                                d[u]["parrots"] = sorted(d[u]["parrots"], key=lambda k: k["name"])
                                if len(d[u]["parrots"]) >= 50:
                                    ach(client.get_user(int(u)), 2)
                                if breed == "Caique":
                                    ach(client.get_user(int(u)), 3)
                                f.seek(0)
                                f.truncate()
                                json.dump(d, f, indent=4)
                                f.close()
...
def backup():
    with open(os.path.join(sys.path[0], "currency.json"), "r+") as f:
        with open("backups/b_currency.json", "r+") as g:
            if len(f.read()) >= len(g.read()):
                shutil.copyfile("currency.json", "backups/b_currency.json")
        f.close()
        g.close()
    with open(os.path.join(sys.path[0], "memberlists.json"), "r+") as f:
        with open("backups/b_memberlists.json", "r+") as g:
            if len(f.read()) >= len(g.read()):
                shutil.copyfile("memberlists.json", "backups/b_memberlists.json")
        f.close()
        g.close()
    shutil.copyfile("preferences.json", "backups/b_preferences.json")
    with open(os.path.join(sys.path[0], "quotes.json"), "r+") as f:
        with open("backups/b_quotes.json", "r+") as g:
            if len(f.read()) >= len(g.read()):
                shutil.copyfile("quotes.json", "backups/b_quotes.json")
        f.close()
        g.close()
    shutil.copyfile("quotescount.json", "backups/b_quotescount.json")
    with open(os.path.join(sys.path[0], "quotesoffensive.json"), "r+") as f:
        with open("backups/b_quotesoffensive.json", "r+") as g:
            if len(f.read()) >= len(g.read()):
                shutil.copyfile("quotesoffensive.json", "backups/b_quotesoffensive.json")
        f.close()
        g.close()
    time.sleep(60)
